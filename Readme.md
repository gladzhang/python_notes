...
补充：
> staticmethod和classmethod都是类函数，可以直接通过类名调用（即不需要实例化对象）；而staticmethod则不需要传递self参数，可以直接调用；classmethod经常用到通过外部传递参数来构造对象，因此本身会有cls参数，用传递的参数构建自身的类实例，常见于ClassA.from_pretrain()函数
